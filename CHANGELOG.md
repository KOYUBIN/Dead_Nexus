...
### v6.55 — 협동 시나리오 모드 "INCURSION" 신설 (80차, 유저 요청: 정령섬·엘드리치류 시나리오 깨기)
- **신규 모드 축**: 솔로/핫시트와 나란히 **🤝 협동 (INCURSION)** — 전 좌석(인간+아군 봇)이 한 팀으로 MERIDIAN 청산관리단과 싸운다. RPG Act 2~3 정본 세력의 시뮬 역수입(3트랙 세계관 교차, [계승] 태그 — ASSESSOR/COLLECTOR/LIQUIDATOR·"장부·담보·회수" 어휘)
- **위협 엔진 (`coop_module.js` 신규, 전량 모듈 외장)**: ① 침공 시간표 — 어느 구역에 몇 R에 상륙하는지 **셋업에서 전부 공표되는 결정론 시간표**(정보 은닉 없음이 이 모드의 정체성, S07 설계 언어 계승·Math.random 0) ② **파멸 트랙(RECEIVERSHIP 게이지)** — 활성 거점·상륙마다 상승, 거점 파괴·Bloc 장부 방어로 하강, 상한 도달 = 즉시 팀 패배 ③ 비대칭 협동 — Ghost는 강습(결정론 판정, d6 없음), Bloc은 자산·복구·방어로 기여
- **시나리오 3종 + 별점·해금** (`COOP_SCENARIOS` 별도 레지스트리 — 경쟁 S01~S07과 분리): C01 상륙(거점 4 전파괴) → C02 압류 집행(핵심 구역 10R 사수) → C03 정산일(기함급 지휘 거점 격파, C01·C02 클리어 시 해금 — RPG a3-03과 세계관 호응). 별점 1~3★ 판정식 공개, `dn_coop_v1` 영속 기록
- **아군 봇 협동 조준**: 활성 거점 좌석순 분담 배정(결정론 그리디) + 협동 반쪽 선택 — v6.54 `rules_botGoalGap` 선례의 목표 함수 파생. PvP 게이트 전면: 이동 레이드/결투/적대 M&A/견제/적대 카드 51키 무력화·개인 승리 차단
- **밸런스 실측** (봇 전용 자동 협동 n=150/시나리오): C01 51.3% · C02 44.7% · C03 68.7% — 전 셀 목표 대역(40~70%) 안착. 튜닝 이터레이션·봇 결함 3건 교정 근거는 docs/26 §4. **설계 이탈 1건 정직 기록**: 강습 사거리 온셀 → 온셀+4방 인접(포위 강습) — 온셀 한정은 봇 도달 정밀도 병목으로 클리어 0%(docs/14 §S04 교훈 재현)
- **격리 증명**: 협동 off 시 기존 모드 참조 동일 항등 유닛 + S01 솔로 n=150 ghost 46%(밴드 내) — 경쟁 밸런스 불변. 기존 유닛 563 전부 유지
- 홈: 시뮬 카드에 협동 지표·문구 추가, `_home_check.js` 가드 14b 신설(홈 협동 배열 == coop_module 레지스트리 키) — **38항**
- docs/26-coop-mode.md 신규(설계·판정식·측정·알려진 한계 §6)
- 시뮬 유닛 **629**(신규 66, ×3회) · rpg 443 · 홈 38/38 · babel 495,739자(<500K) · 모듈 8종 로더·file:// 검증 · 외부 요청 0

### v6.54 — 대규모 업데이트: RPG Act 3 · 시뮬 S07 · 홈 재설계 · 미감사 영역 첫 감사 (79차)

**⚔ RPG — Act 3 "SIGNAL DEBT" 신설 (미션 32 → 38종, 완주 ~4.4h → ~5.2h)**
- 콘셉트: 기함이 격추된 뒤 남은 것은 군대가 아니라 **장부**다. MERIDIAN이 청산되자 채권자 청산관리단이 담보를 회수하러 온다 — 담보 목록에는 메시 인프라·코어텍스 인증망·시민 접속 권한, 그리고 SIGNAL 자신이 올라 있다. 화자는 lore 정본의 BROKER = SILK(Act 2 의뢰인이 장부의 주인으로 승격)
- 메인 체인 4종 (`a3-00-framing` → `a3-01-collateral` 2연전 → `a3-02-interest` 2연전 → `a3-03-finale` **3연전**, 캡스톤 클리어 시 해금). 피날레는 a3-01/02 선택 플래그를 읽는 회고 5선택 + 최종 보스 `MERIDIAN_LIQUIDATOR`
- 클래스 사이드 2종: `a3-side-broker-callin`(장부 소유자 BROKER — a2-side-broker-ledger 플래그 계승) · `a3-side-rigger-relay`(담보 3번 항목 = 메시 중계망, RIGGER 원전 명대사가 곧 해법). 나머지 4클래스는 Act 2 사이드로 개인 서사 기완결
- 신규 적 3종(사정관·추심관·청산인 — 기존 밸런스 대역 내, 스프라이트 재사용으로 신규 아트 0). **신규 장비 0** — 장비 추가는 `full` 시나리오 밸런스 핀을 오염시키므로 의도적 배제
- 밸런스: 252 → **312조합 전수 클리어**(clearFail 0·trivial 0·attrition 0). **hard×base 실패 21건이 기존 핀 집합과 byte 동일 — Act 3 신규 실패 0건**
- 엔딩 4종 계약 무편집(`ENDINGS`/`ORDER`/`DERIVE_ORDER` 불변, Act 3 종결은 `returnHub`라 엔딩 기록에 영향 없음 — 유닛 핀)

**🎮 시뮬 — S07 「블랙아웃 카스케이드」 신설 + 봇 AI 승리 게이트 수정**
- S07 정체성: 위협이 **무작위가 아니라 공표된 시간표**. 어느 구역이 몇 R에 꺼지는지 시작 전부터 확정(18칸 고정 순서) — 6종 중 유일하게 "선점·대비"가 전략이 되는 판. 정체성 룰 5종(결정론 캐스케이드·정전 수입 0·정전 레이드 방어 −2·Bloc 복구 경제·계단식 붕괴 압력) 전부 수치 리터럴 유닛으로 핀. 실측 ghost 43.8%·avgR 7.46로 기존 6종 대역 내 안착
- 설계 기각 2건 정직 기록: 이동 차단(봇 BFS에 재탐색 계층 없음 + S04 구출 퀘스트 0.26회/판 사문화 전례) → "정전 구역은 오히려 뚫린다"로 축 반전 / 매R 붕괴 압력(S04와 정체성 중복) → 계단식 유계화
- **[실결함] 봇 AI가 승리 게이트를 오독**: 승리 근접 판정이 하드코딩 상수(`16 - rep`, `2 - raids`)여서 ① 16이 실임계의 23% 지점이라 "임박" 신호가 상시 참 ② 인원수 보정·언더독 스케일·`ghostRaidsOverride`를 전부 무시 — **S03에서 레이드 게이트 미충족을 충족으로 오판**. 판정·HUD와 동일한 진척 함수를 재사용하는 `rules_botGoalGap`으로 교체 → 레이드/판 전 시나리오 28~47% 상승, S03 ghost 61.8→70.4%(docs/14 명시 상한에 안착), 붕괴 0
- **`scenario_module.js` 신설** — S07 엔진 + 인라인에서 이전한 S06 타이틀·initGame 훅·언더독 스케일. 인라인 블록 499,169 → **495,633자 순감**(여유 831 → 4,367)

**🏠 홈 포털 재설계 — 게이트 12항 → 37항**
- 첫 방문자 온보딩: no-JS 마크업 자체가 신규 방문자 뷰이고, 세이브 감지 시 "이어하기 → 다음 미션명 + 진행 요약"으로 교체(RPG 진행 > 시뮬 진행 > 신규 우선순위). 파싱 실패·스키마 이상은 전부 `console.warn` 후 신규 뷰 폴백
- 트랙 카드 지표 7종 전부 배열 length 파생(하드코딩 0) — **이 가드가 실제 드리프트를 즉시 검출**(시나리오 6 → S07 신설로 7, 미션 32 → Act 3로 38)
- 그래픽(코너 브래킷·헤일로 호흡·카드 인덱스·CTA 해치, 전부 CSS·reduced-motion 존중) · PWA shortcuts 3종 + maskable 아이콘 · 접근성(대비 30쌍 자동 검사, `outline:none`으로 지워져 있던 **포커스 링 복구**, 랜드마크/헤딩 정리)
- `_home_check.js`가 홈 파일 검사기에서 **프로젝트 전역 정합 게이트**로 승격: 홈이 표시하는 미션·클래스·시나리오·챕터 수가 각 트랙 코드 배열과 동치인지, 세이브 키가 실제 상수와 같은지까지 검사. 부수로 기존 주석 제거 로직이 줄 주석 속 `/*`에 걸려 **뒤 검사를 통째로 삼키던 버그**를 발견·수정

**🖨 print-kit·cards·sim-harness 첫 전면 감사 — 결함 17건 수정**
- **11×11 정식 맵의 Bloc HQ 5곳·Ghost 시작 6곳 좌표가 전부 정본과 불일치** — 인쇄해서 플레이하면 설계와 다른 맵이 되는 수준. `docs/10` 정본으로 ZONES·셋업 표·헤더·블러브 전량 정정
- CARBON 덱 TL 순서 역전 · 속성 토큰 "6종 60개" → 실제 7종 70개 · 캐릭터 시트 매수 누락(CYBERWARE 시트가 계산에서 빠짐) · `07-reference` 내부 자기모순 2건(특수 구역 소유권·NEXUS 승리 2R/3R) · S01 NEXUS 조건 맵별 표기 누락
- cards: 숨은 목표 난이도 집계 오류 2건(실측 5/9/6) · 레거시 신규 카드명이 기존 카드와 충돌 2건(LAST STAND·GHOST PROTOCOL) 개명 · 섹션 참조 오류 2건. docs 사실오류 2건(존재하지 않는 경로·Ghost 덱 구성표)
- sim-harness 도구 5종 전부 실행 가능 확인. README가 구버전 CLI를 안내하고 core.js 재추출 명령이 파일 성장으로 깨져 있어 정정 + "알려진 한계" 절 신설(core.js는 FROZEN이라 미수정)
- **동결 방침 내 발산은 손대지 않음** — print-kit v1은 보드게임 스냅샷이므로 시뮬 v6.x 수치와의 차이는 결함이 아니라 방침. 미해결 설계 결정(Bloc 공용 카드 3중 불일치)도 창작 범위라 기록만

**📓 노션 동기화** (커넥터 재연결): 허브 현재 버전 v6.44 → v6.53 · 밸런스 DB **B-01 행 Done/기각 종결** · 협업 페이지의 P0 위험 4건 재평가(종료 점수 판정·모듈 조용한 비활성화·문서 정합성·테스트 사각지대 해소 확인, 사람 플레이 데이터 부재만 유효 공백으로 존치)

- rpg 유닛 443 · 시뮬 유닛 **563**(신규 67, ×3회) · 미션 38/38 · 배선 11/11(38종 4집합) · 홈 37/37 · 배경 312조합 · babel 양 트랙 무에러 · 순수성/결정론 grep 0

### v6.53 — 자율 업데이트 8차분: RPG 유닛 커버리지 보강 + 결함 2건 수정 (78차)
- **유닛 커버리지 보강 (406 → 431, +25)**: ① 리듀서 22액션 전수 인벤토리 리터럴 핀 (신규 액션 무단 추가 시 즉시 실패) + 미커버 7종 봉합 (NEW_GAME 엔딩 기록 계약·COMBAT_SELECT·SPEND_KARMA 미러 동기·HUB_NAV 등) + 디스패치 페이로드 키 계약 10종 (UI↔store 실결합면 — 코어 루프를 디스패치만으로 완주) ② dialogueChoose 실경로 (a2 2연전 실전이·hardScale 인카운터별 우선 조회·복합 비용 사유 우선순위) ③ 전투 경계 (가드 identity 5종·전투 중 세이브 byte 불변+왕복 무손실) ④ abyss 무효 웨이브·byClass floor·best 승격 ⑤ campaignStats 분류 경계 ⑥ migrate 전방호환·heat 클램프·손상 입력 ⑦ 동어반복 2건 교정 (자기검증 → 독립 리터럴) ⑧ 사문 필드 (hackOnly·crew·hubState) grep 근거 주석 핀
- **[테스트가 발견한 결함 2건 → 즉시 수정]** ① 전투 4액션 (COMBAT_MOVE/ATTACK/HACK/END_TURN) combat null 가드 부재 — 비전투 씬 디스패치 시 TypeError (SELECT/CLEAR_FLOATERS 는 가드 보유였던 비대칭) → identity 가드로 대칭 복원 ② save.migrate 가 배열 JSON 을 수용 (typeof 'object' 만 검사) — 배열 세이브는 백필 속성이 재직렬화에서 전부 소실되는 손상 경로 → Array.isArray 거부 (abyss.js 선례). 핀 테스트 2건을 수정 회귀로 전환
- rpg 유닛 431/431 ×3회 · 스모크 12/12 · 배선 11/11 · 프로덕션 변경은 결함 수정 2파일 (store.js·save.js) 각 수줄뿐

### v6.52 — 자율 업데이트 7차분: 시뮬 유닛 커버리지 대보강 + 협상 stale 결함 수정 (77차)
- **유닛 커버리지 대보강 (404 → 496, +92)**: 3차 감사가 실측한 사각지대 전량 봉합 — ① 승리 선언 파이프라인 최초 커버 (R5 가드·선언·2회 캡·견제 해제·익라운드 확정·M&A 즉시 확정, NEXT_ROUND 실배선 플로우 포함 13건) ② M&A 엔진 28건 (선언 체크 사유 전종·resolveMna 3분기 — 인수 완료/방어 재매입/NPC 껍데기·지분 defeated/whiteKnight 분기·방어 결정 5종 수치 리터럴) ③ 협상 10건 ④ 스캔들 가드·숏 정산 전체 흐름 8건 ⑤ assetValue 구역·건물 가산 분기 실행 4건 ⑥ 리듀서 핵심 액션 18건 (EXECUTE_TURN 이동/레이드/카드·NEXT_ROUND 전환/만료·COLLECT_INCOME) ⑦ 동어반복 테스트 9건 교정 (프로덕션 식 재계산 → 독립 수동 리터럴, S02 정체성 룰 동작 어서션)
- **[테스트가 발견한 실결함 → 즉시 수정] NEGOTIATE_PHASE stale 스냅샷**: 봇 협상 후보 생성이 페이즈 시작 스냅샷 기준이라, 선행 스왑으로 자원이 바뀐 봇이 보유량 초과 give 제안을 낼 수 있었음 (자원 음수 경로). 수정: ① 후보 생성을 매 반복 최신 상태 기준으로 ② 같은 쌍 재스왑 차단 (수정만으로 노출되던 핑퐁 왕복·인맥 트랙 공짜 상승 방지) ③ negoApply에 give 보유량 방어층 (부족 시 "협상 무산" 로그 — 조용한 음수 차감 원천 차단). 회귀 유닛 2건
- 감사 원지목 중 DECLARE_VICTORY/USE_CARD 액션명은 실존하지 않음 확인 — 실경로 (NEXT_ROUND 내 선언 파이프라인·EXECUTE_TURN 카드 경로)로 봉합, 오탐 정직 기록
- Math.random 상수 스텁 (withRand) 으로 확률 게이트 전부 결정론화 — 3회 연속 ALL PASS, 플레이키 0
- **[임계 주의]** 인라인 babel 블록 499,169자 (여유 831자) — 다음 시뮬 코드 추가는 반드시 모듈 외장부터
- 시뮬 유닛 496/496 ×3회 · 홈 12/12 · babel 무에러 · 프로덕션 변경은 협상 결함 수정 2파일뿐
