'use strict';
// ============================================================================
// [RPG SNAPSHOT — read-only vendoring, docs/25 §4.3 G3]
//   이 파일은 simulator/v0.5/lore_module.js 의 READ-ONLY 스냅샷 복제본이다.
//   원본은 절대 수정하지 않으며, RPG 트랙은 이 스냅샷만 참조한다(lore-adapter 경유).
//   원본 대비 유일한 추가: 파일 하단에서 BLOC_IDENTITY·GHOST_IDENTITY 를 window 로
//   재노출한다. 원본은 이 둘을 IIFE 내부 var 로 감춰 window 도달 불가(=undefined 버그,
//   §4.3 실측 정정). 어댑터가 정체성 산문을 쓰려면 스냅샷에서 재노출해야 하기 때문이다.
//   (게임 로직 무변경 — 표시 전용 상수/헬퍼만 존재.)
// ============================================================================
// ---- (이하 원본 lore_module.js 전문) ----
...
(function (glob) {
  ...
  // [RPG SNAPSHOT 추가 노출, G3] 정체성 산문 재노출 (원본은 window 미도달).
  glob.LORE_BLOC_IDENTITY = BLOC_IDENTITY;
  glob.LORE_GHOST_IDENTITY = GHOST_IDENTITY;
  // node(require) 지원 — 브라우저에선 무해.
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { LORE_GHOSTS: LORE_GHOSTS, LORE_BLOCS: LORE_BLOCS, loreTag: loreTag,
      loreQuote: loreQuote, loreEpilogue: loreEpilogue, loreRouteFromReason: loreRouteFromReason,
      LORE_BLOC_IDENTITY: BLOC_IDENTITY, LORE_GHOST_IDENTITY: GHOST_IDENTITY };
  }
