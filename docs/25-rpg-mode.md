...
**버전**: v1.1 (현행화 — Stage 1~3 전부 구현 완료 + Act 2·캡스톤·심연 프로토콜·6클래스 완성·오브젝티브 다양성 반영, v6.46 기준. 69차)
**최초 작성**: 2026-07-17 (v0.1, 비전 초안 — 수직 슬라이스 확정 전)
**최종 수정**: 2026-07-26
**아키텍처 코드네임**: GHOSTGRID (hybrid-scene)
**관련 문서**: `docs/01-worldbuilding.md`, `docs/03-factions-blocs.md`, `docs/04-characters-ghosts.md`, `docs/06-attribute-system.md`, `docs/07-combat-stats.md`, `docs/09-tech-tree.md`, `docs/12-legacy-campaign.md`, `docs/22-game-identity.md`, `cards/legacy/chapter-01-first-blood.md`, `simulator/v0.5/lore_module.js`, `rpg/README.md`, `CHANGELOG.md`(v6.29~v6.46)

> **현행화 안내**: §1~§6은 v0.1 비전 초안(2026-07-17, Stage 0 시점) 원문을 **그대로 보존**한다 — 아키텍처 결정 근거와 계보 표는 착수 당시의 설계 기록으로서 사후에도 유효하다. §7(로드맵)에 완료 상태·각주를 덧붙였고, §8(비-목표)에 해소/유지 상태를 표기했으며, **§9** 가 v6.46 기준 실제 구현 현황을 사실관계로 정리한다. 원문 vs 실장 괴리는 §7·§9의 각주로만 기술하며, §1~§6 본문은 편집하지 않는다(계획 문서로서의 사료 가치 보존). v6.44 현행화(64차)에서는 원 로드맵(Stage 1~3, ~v6.40)에 없던 **Act 2 "AFTER ZERO DAY"(v6.43)·캡스톤+심연 프로토콜+사운드(v6.44)** 를 §7 로드맵 말미와 §9에 "계획 대비 초과 확장"으로 정직 각주 처리했고, v6.46 현행화(69차)가 **6클래스 완성·오브젝티브 다양성(v6.45~v6.46)** 을 동일한 방식으로 추가 각주 처리했다.
...
> **각주(계획 초과 확장)**: 클래스 로스터는 Stage 2 문서상 "1~2종"이 목표였으나, 이후 **v6.34(48~49차)에서 RIGGER·MOLE 2종이 추가 편입**되어 플레이어블 4클래스(CIPHER·BLADE·RIGGER·MOLE)로 확장되었고, **v6.45(64~65차)에서 BROKER·DRIFTER가 마저 플레이어블로 승격**되어 최종 **플레이어블 6클래스(전량)**로 완성되었다(`CLASSES.PLAYABLE` 6개, `rpg/_unit.js` #98). 이는 원 로드맵의 Stage 2/3 어디에도 명시되지 않은 **추가 확장**이며, §8 Non-goals의 "6클래스 전체 로스터" 배제 원칙은 **v6.45에 완전 해소**됐다. 상세는 §7 v6.45 각주·§9.
...
### Stage 3 이후 — 추가 확장: 6클래스 완성 + 오브젝티브 다양성 + B-01 종결 (v6.45~v6.46, 64~69차)

> **정직 고지**: 아래 항목도 §2(비전)·Stage 1~3 원 로드맵 밖의 확장이다. v6.44 캡스톤 완결 이후 유저 "할 일들 찾아서 업데이트"(v6.45) 및 자율 업데이트 루프(v6.46)로 착수된 범위다.

- **v6.45 — RPG 6클래스 완성** (64~65차): BROKER(SILK — BLACKMAIL·POKER FACE·INFO BROKER·BURN THE BRIDGE 은신 3턴, 무소음 SHADE 원격)·DRIFTER(FLINT — RAM CHARGE·AMBUSH·GHOST RUN, ASH 브루저)를 `broker.md`/`drifter.md` 원전 킷 계승으로 플레이어블 승격 + 클래스 전용 사이드 미션 2종 추가 → **미션 32종**, **플레이어블 6클래스(전량)**. 밸런스 252조합 clearFail 0(BROKER 저HP 회피형 보정 6건 — 미션 데이터만). 하네스 하드모드 축 신설. rpg 유닛 285/285.
- **v6.46 — 오브젝티브 다양성 + S06 타이틀 + B-01 종결** (66~69차): **생존형 승리조건 `combat.survive: N`**(엔진 4줄 diff, 미사용 시 byte 불변) — `a2-c1-first-contact` 외벽 방어전에 적용, 6클래스 대표 경로가 전멸/차감/사수 3갈래로 분화. **HACK 전용 코어** — `side-02-corp-breach` 오브젝티브를 `ICE_NODE` 5기 격자로 물화(물리축은 전멸 폴백, 엔진 무변경 데이터 레이어). blade-vendetta 사문 게이트를 `[IRONWALL 태그]` → `[DEF 4]` 축으로 교체. **S06 재건왕/청산자 타이틀 배선**(69차, §S06 참조). **B-01 종결**(66차, 시뮬 트랙): S03 2-레버(자산×렙) 전수 측정으로 공집합 확정 기각(`playtesting/balance-issues-digest.md` §3). **MOLE HELIX 태그**: `mole.md` Card01·검체 E-7 원전 근거 개방. 유닛 321/321·미션 검증기 32/32·밸런스 하네스 252/252.
...
- 6클래스 전체 로스터(슬라이스는 CIPHER 단독) · 시그니처 풀 매트릭스. → **✅ 해소 (v6.45)**: 플레이어블 클래스가 CIPHER→4종(v6.34, CIPHER·BLADE·RIGGER·MOLE)을 거쳐 **6종 전량**(v6.45, +BROKER·DRIFTER)으로 완성됐다. BROKER(SILK, 중개·은신)·DRIFTER(FLINT, 기동·차량돌진)는 각 킷을 `broker.md`/`drifter.md` 원전에서 계승했고, `CLASSES.PLAYABLE`(6개) + 클래스 전용 사이드 미션 2종이 함께 편입됐다(`rpg/_unit.js` #98이 6종 고정을 회귀 방어). "전체 6클래스" 배제 원칙은 완전 해소.
...
§7·§8의 각주에서 다룬 이탈·확장을 종합해, v6.46(2026-07-26 기준 main HEAD `565786f`) 시점 RPG 트랙의 **실제 시스템 인벤토리**를 사실관계로 정리한다. 아래 수치는 전부 `rpg/` 코드·`rpg/_unit.js`·`rpg/_missions_check.js`·`rpg/_balance.js` 실행 결과(2026-07-26 재실행 재확인: 유닛 321/321·미션 검증기 32/32·밸런스 하네스 252/252)·CHANGELOG.md v6.29~v6.46 항목에서 직접 확인했다(§ 문서 하단 교차 확인 표 참조).

### 9.1 시스템 인벤토리

| 시스템 | 수량 | 구성 | 근거 |
|---|---|---|---|
| **미션** | 32종 | 메인 챕터 1~8(`ch01`~`ch08`) + 사이드 계약 8종(`side-01`~`side-08`, v6.33, 계 16종) + **Act 2 "AFTER ZERO DAY"** 13종(프롤로그 `a2-00` 1 + 엔딩 4갈래 후일담 2연전×4=8 + 클래스 개인 서사 4, v6.43) + **캡스톤** `a2-99-flagship` 1종(v6.44) + **BROKER/DRIFTER 클래스 사이드** 2종(`a2-side-broker-ledger`·`a2-side-drifter-lastroad`, v6.45) | `rpg/data/missions/*.js` 32개 파일, `node rpg/_missions_check.js` 32/32 PASS(재실행 확인) |
| **플레이어블 클래스** | 6종 (전량, v6.45 완성) | CIPHER(HACK 특화)·BLADE(근접·최고 HP/ATK)·RIGGER(설치·최고 DEF)·MOLE(위장·무소음)·BROKER(SILK, 중개·최고 SPD·은신 회피형)·DRIFTER(FLINT, 기동·고HP+고SPD 브루저) | `rpg/data/classes.js` `CLASSES.PLAYABLE`, `rpg/_unit.js` #98 (PLAYABLE.length===6) — v6.34 4종 → v6.45 BROKER·DRIFTER 편입으로 6종 완성, 이후 증감 없음 |
| **장비** | 10종 (불변) | 무기 개조 4(스마트 링크·아이스 브레이커·모노 엣지·헤어 트리거) + 사이버웨어 6(무드 칩·광학 임플란트·리플렉스 부스터·미오머 다리·강철 피부·뉴럴 잭) | `rpg/data/gear.js` `ITEMS`, 슬롯 2종(weapon/cyberware), 전부 스탯 델타만(신규 메커닉 0) — v6.38 이후 증감 없음 |
| **엔딩** | 4종 (불변) | 🏙️CORPORATE ETERNAL(지배)·🔥STREET RISING(영웅)·🕊️NEXUS REBORN(합일)·💀DEAD NEXUS(기본, 폴백) — Act 2는 이 4종의 후일담 갈래이지 신규 엔딩 추가가 아님 | `rpg/systems/ending.js` `ENDINGS`(키 4개) + `ORDER` 배열 |
| **적 로스터** | 31종 (23→+8) | 기존 23종 + **MERIDIAN**[신규 세력] 7종(Vanguard·Stalker·Drone·Warlord + Act2 무대) + 캡스톤 보스 MERIDIAN_OVERLORD[신규] 1종(v6.43~44) | `rpg/data/enemies.js`, `node rpg/_missions_check.js` 화이트리스트 "알려진 31종" 출력(재실행 확인) |
| **캡스톤** | 1종 (신규, v6.44) | "MERIDIAN FLAGSHIP"(`a2-99-flagship`) — 4갈래 최종 미션 전부 클리어 시 해금, 3연전 최종 결전 + 갈래별 선택 회고 + 전용 에필로그 "ASHGRID PREVAILS" | `rpg/data/missions/a2-99-flagship.js`, `rpg/systems/campaign.js` capstone 게이트 |
| **심연 프로토콜** | 1종 (신규, v6.44) | 무한 상승 웨이브 계약 — 결정론 웨이브 풀 순환·스케일 1+0.05N·로컬 최고 기록(`abyss.best`), 캡스톤 클리어가 해금 게이트 | `rpg/systems/abyss.js`, `rpg/_unit.js` #259~265 |
| **사운드** | RPG 12종 / 시뮬 11종 (신규, v6.44) | Kenney CC0(Interface/Sci-Fi/Digital Audio 팩), 마스터 🔊/🔇 토글(localStorage 분리 키), 전투 엔진 순수성 유지(표시층 훅만) | `rpg/assets/audio/*.ogg`(12개, 재실행 확인) · `simulator/v0.5/audio/*.ogg`(11개, 재실행 확인) · 각 `LICENSE.md` |
| **검증 인프라** | 3종 | ① `rpg/_unit.js` 순수 로직 유닛(321/321 PASS) ② `rpg/_missions_check.js` 미션 스키마·대화 그래프 검증기(32/32 PASS) ③ `rpg/_balance.js` 전투 밸런스 하네스(6클래스 × 42 인카운터 = 252조합 전수 클리어, clearFail 0, `--smoke` 결정론 재현 PASS) | 각 스크립트 직접 실행 결과(2026-07-26 재확인) |

**부가 확인 사실**(표에는 없으나 §7·§8 각주 근거로 실측):
- ~~비활성 클래스 데이터 2종(BROKER·DRIFTER)이 `PLAYABLE`에는 없음~~ **(v6.45 해소)**: BROKER·DRIFTER가 `CLASSES.PLAYABLE`에 편입되며 6클래스 전량이 활성 상태다 — §9.2 "향후 후보"에서도 제외됨.
- **오브젝티브 다양성 (v6.46, 68차)**: 기존 "차감형/전멸형 단일 축" 미션 구조에 `combat.survive: N`(생존형, `a2-c1-first-contact`)와 HACK 전용 코어(물리 무효 ICE 격자, `side-02-corp-breach`)가 데이터 레이어만으로 추가됐다. 나머지 미션의 밸런스 매트릭스는 byte 불변.
- **B-01(S03 평균 라운드 미달) 기각 종결 (v6.46, 66차)**: `blocAssetBonus`×`ghostRepBattleOverride` 2-레버 9셀 + 레이드 게이트 보충 4셀 n=300 전수 측정으로 (ghost≤70 ∩ avgR≥8) 공집합을 확정, 구속항은 렙이 아니라 레이드 게이트임을 규명(`playtesting/balance-issues-digest.md` §3 참조). RPG 트랙 자체 변경은 없음(시뮬 트랙 이슈).
- New Game+(v6.40 도입, v6.44 확장): 진행(미션·flags) 리셋, **엔딩 기록(seen/byClass/runs) + 캡스톤 기록(capstone/capstoneByClass) + 심연 최고 기록(abyss.best)** 이 함께 영속 이월, 구세이브 마이그레이션 멱등(`rpg/_unit.js` #267~268).
- Act 2 갈래 개방은 `endings.seen` 기반이며 회차(NG+)마다 누적 — 4엔딩 전부 열람해야 Act 2 완전 개방(§7 v6.43 각주 참조).
...
- **BROKER·DRIFTER 클래스 활성화 — ✅ v6.45에서 실장 완료 (더 이상 미착수 후보 아님)**: `broker.md`/`drifter.md` 원전 킷 계승으로 시그니처 킷 설계 + 미션 화이트리스트 확장을 완료, `CLASSES.PLAYABLE` 6종 전량이 됐다(§8 "6클래스 전체 로스터" 참조). 본 항목은 사실관계 갱신을 위해 남겨 두되, 실장 완료로 향후 후보 목록에서 제외한다.
- **"After Zero Day" 확장 후크 — ✅ v6.43에서 실장 완료 (더 이상 미착수 후보 아님)**: 구버전(v6.40 현행화 시점)에는 `rpg/systems/ending.js`의 `nexus-reborn` 엔딩 대사에 인게임 텍스트로만 존재하던 서사적 후크였으나, **v6.43 "RPG Act 2 AFTER ZERO DAY"** 가 이 후크를 실제 미션 13종(§9.1)으로 구현했다 — 원전 정본 "After Zero Day"(엔딩 4갈래 후일담) 아크로 명시 배선. 본 항목은 사실관계 갱신을 위해 남겨 두되, 실장 완료로 향후 후보 목록에서 제외한다.
- **Act 2 이후 신규 무대 확장**: 메인 캠페인은 챕터 1~8(레거시 캠페인 8/8 완결과 동일 아크) + Act 2 "AFTER ZERO DAY"(엔딩 후일담·클래스 개인 서사·캡스톤)까지 구현되어 있다. 캡스톤 이후("Act 3" 등)에 대한 데이터 파일·계획은 리포에 없다. `systems/*` 엔진이 "데이터 파일 추가만으로 확장"(§7 Stage 3 수용 기준)되도록 설계되어 있다는 사실만 유효하며, 그 이상의 확장 여부는 미결정이다.
- **§8에서 여전히 "⏳ 유효"로 남은 Non-goals 전 항목**(리액션/오버워치/기회공격/수직성/파괴 가능 엄폐, 캔버스 렌더, 다중 상태이상, 소환/드론 액터, 다중 벤더/리스펙/다중 허브, 온라인/서버 세이브) — §8 배제 원칙이 유지되는 한 착수 대상이 아니다. 심연 프로토콜(v6.44)은 신규 상태이상·소환 액터 없이 기존 로스터의 결정론 스케일링 반복 스폰으로 구현되어 이 배제 원칙과 충돌하지 않는다(§7 각주 참조).
- **실사용자 플레이 피드백 반영 사이클**: v6.29~v6.44 CHANGELOG 어디에도 RPG 트랙에 대한 외부 실플레이 피드백(플레이 후 난이도·재미·UX 보고)을 원인으로 한 변경 이력은 없다(모든 변경은 내부 밸런스 하네스·유닛테스트·Playwright 자동 검증 기반이거나 프로젝트 관리자의 개발 스코프 요청). `COLLABORATION.md` §6도 "실플레이 피드백 대기"를 여전히 최우선 미해결 입력원으로 명시한다. 실사용자 피드백 반영은 v6.44까지 별도 사이클로 진행된 바 없다.
...
*§1~§6·부록은 v0.1 비전 초안(2026-07-17, Stage 0 시점) 원문이며 사료로 보존한다. §7~§9(v1.1, 2026-07-26 현행화)가 v6.29~v6.46 실장 결과를 반영한다 — Stage 1~3(v6.29~v6.40)은 원 로드맵대로, Act 2·캡스톤·심연 프로토콜·사운드(v6.43~v6.44)와 6클래스 완성·오브젝티브 다양성(v6.45~v6.46)은 원 로드맵에 없던 계획 외 확장으로 각각 §7·§9에 각주 처리했다. `data/*` 파일의 계보 표(`[계승]`/`[각색]`/`[신규]`)는 각 파일 주석에 상시 유지되고 있으며, 수치는 `docs/07`·`docs/06` 원전과 대조 감사된 상태다(§3.1 계보 표 참조).*