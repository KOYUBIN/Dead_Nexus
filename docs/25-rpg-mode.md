...
§7·§8의 각주에서 다룬 이탈·확장을 종합해, v6.44(2026-07-21 기준 main HEAD `55ae9da`) 시점 RPG 트랙의 **실제 시스템 인벤토리**를 사실관계로 정리한다. 아래 수치는 전부 `rpg/` 코드·`rpg/_unit.js`·`rpg/_missions_check.js`·`rpg/_balance.js` 실행 결과(2026-07-24 재실행 재확인)·CHANGELOG.md v6.29~v6.44 항목에서 직접 확인했다(§ 문서 하단 교차 확인 표 참조).

### 9.1 시스템 인벤토리

| 시스템 | 수량 | 구성 | 근거 |
|---|---|---|---|
| **미션** | 30종 | 메인 챕터 1~8(`ch01`~`ch08`) + 사이드 계약 8종(`side-01`~`side-08`, v6.33, 계 16종) + **Act 2 "AFTER ZERO DAY"** 13종(프롤로그 `a2-00` 1 + 엔딩 4갈래 후일담 2연전×4=8 + 클래스 개인 서사 4, v6.43) + **캡스톤** `a2-99-flagship` 1종(v6.44) | `rpg/data/missions/*.js` 30개 파일, `node rpg/_missions_check.js` 30/30 PASS(재실행 확인) |
| **플레이어블 클래스** | 4종 (불변) | CIPHER(HACK 특화)·BLADE(근접·최고 HP/ATK)·RIGGER(설치·최고 DEF)·MOLE(위장·무소음) | `rpg/data/classes.js` `CLASSES.PLAYABLE`, `rpg/_unit.js` #98 (PLAYABLE.length===4, BROKER/DRIFTER 제외 회귀 고정) — v6.34 이후 증감 없음 |
...
| **검증 인프라** | 3종 | ① `rpg/_unit.js` 순수 로직 유닛(271/271 PASS) ② `rpg/_missions_check.js` 미션 스키마·대화 그래프 검증기(30/30 PASS) ③ `rpg/_balance.js` 전투 밸런스 하네스(base/mid/full 무장비·하위장비·최고가 3정책 × 전 미션·클래스 조합 = 160/160 전수 클리어, `--smoke` 결정론 재현 8/8 PASS) | 각 스크립트 직접 실행 결과(2026-07-24 재확인) |

**부가 확인 사실**(표에는 없으나 §7·§8 각주 근거로 실측):
- 비활성 클래스 데이터 2종(BROKER·DRIFTER)이 `data/classes.js`에 정의되어 있으나 `PLAYABLE`에는 없음 — §9.2 참조 (v6.34 이후 상태 불변).
...
- **BROKER·DRIFTER 클래스 활성화**: 스탯 데이터(`data/classes.js`)는 이미 존재하나 `CLASSES.PLAYABLE`에 미포함(§8 "6클래스 전체 로스터" 항목 참조) — 활성화하려면 시그니처 킷 설계 + 미션 화이트리스트 확장이 추가로 필요(v6.44까지 미착수).
...
*§1~§6·부록은 v0.1 비전 초안(2026-07-17, Stage 0 시점) 원문이며 사료로 보존한다. §7~§9(v1.0, 2026-07-24 현행화)가 v6.29~v6.44 실장 결과를 반영한다 — Stage 1~3(v6.29~v6.40)은 원 로드맵대로, Act 2·캡스톤·심연 프로토콜·사운드(v6.43~v6.44)는 원 로드맵에 없던 계획 외 확장으로 각각 §7·§9에 각주 처리했다. `data/*` 파일의 계보 표(`[계승]`/`[각색]`/`[신규]`)는 각 파일 주석에 상시 유지되고 있으며, 수치는 `docs/07`·`docs/06` 원전과 대조 감사된 상태다(§3.1 계보 표 참조).*